...
export function UserDropdown({ value, onChange, error }: { value: string; onChange: (val: string) => void; error?: string; }) {
  ...
  const { data: usersData } = useUsersList({ searchTerm: debouncedSearch, limit: 10000000000 });
        ...
        <SelectContent className="max-h-60 overflow-y-auto">
          <div className="p-2">
            <Input
              ...
              className="mb-2 w-full border border-accent"
            />
            <Separator />
            {users.length === 0 ? (
