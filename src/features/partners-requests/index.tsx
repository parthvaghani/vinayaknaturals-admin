...
import { columns, type PartnershipRequestRow as TablePartnershipRequest } from './components/columns';
import { DataTable } from './components/data-table';
import PartnershipRequestsProvider from './context/partnership-requests-context';
import { usePartnershipRequestsList, type PartnershipRequest as ApiPartnershipRequest } from '@/hooks/use-partnership-requests';
...
export default function PartnershipRequests() {
  ...
  const { data, isLoading, isError, error, isFetching } = usePartnershipRequestsList({
    page,
    limit,
  ...
  const partnershipRequests: TablePartnershipRequest[] = useMemo(() => {
    ...
    return raw.map((pr) => ({
      ...
      company: pr?.additionalInformation ? String(pr.additionalInformation) : undefined,
      message: pr?.additionalInformation ? String(pr.additionalInformation) : undefined,
        ...
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-y-0 lg:space-x-12'>
          {isLoading || isFetching ? (
            ...
            <DataTable<TablePartnershipRequest, any>// eslint-disable-line @typescript-eslint/no-explicit-any
              ...
              pagination={{ page, limit, total: data?.total ?? partnershipRequests.length }}
