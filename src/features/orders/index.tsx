    ...
    return orders.map((o) => {
      ...
      const derivedPhone =
        typeof userId === 'object' && userId?.phoneNumber
          ? userId.phoneNumber
          : o.phoneNumber
      const originalTotal = (o.productsDetails || []).reduce(
        (sum, p) => sum + p.pricePerUnit * (p.totalUnit || 1),
        0
      )
      const discountedTotal = (o.productsDetails || []).reduce(
        (sum, p) =>
          sum + (p.pricePerUnit - (p.discount || 0)) * (p.totalUnit || 1),
        0
      )
