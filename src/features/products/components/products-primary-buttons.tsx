...
export function ProductsPrimaryButtons() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'description') {
      setDescriptionError(
        value.trim() ? null : 'Please enter a product description.'
      )
    }
  }
  ...
  const handleImagesSelected = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > remainingSlots) {
      toast.warning(
        'You can upload a maximum of 5 images. Extra files ignored.'
      )
    }
  ...
  const handleAddIngredient = () => {
    ...
    if (value) {
      const isDuplicate = productData.ingredients.some(
  ...
  const handleAddBenefit = () => {
    ...
    if (value) {
      const isDuplicate = productData.benefits.some(
  ...
  const handleAddVariant = () => {
    ...
    const isDuplicate = productData.variants[variantType].some(
      (v) => v.weight === weightTrimmed
    )
    if (isDuplicate) {
  ...
  const handleSubmit = () => {
    ...
    const totalVariants =
      productData.variants.gm.length + productData.variants.kg.length
    if (totalVariants === 0) {
          ...
          <div className='max-h-[calc(100vh-100px)] overflow-y-auto p-6'>
            ...
            <div className='mt-6 space-y-5'>
              ...
              <div className='space-y-2'>
                ...
                {ingredientsError ? (
                  <p className='text-xs text-red-500'>{ingredientsError}</p>
                ) : null}
              </div>
                    ...
                    {(['gm', 'kg'] as const).map((type) => (
                      <div key={type} className='mt-2'>
                        ...
                        {productData.variants[type].map((v, i) => (
                          <div
                            key={i}
                            className='bg-muted/50 mt-1 flex flex-col gap-2 rounded border p-2 sm:flex-row sm:items-center sm:justify-between'
                          >
