...
function isUserObject(
  v: OrderRow['userId']
): v is Exclude<OrderRow['userId'], string> {
...
export const columns: ColumnDef<OrderRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const images = (
        (row.original.productsDetails ?? []) as Array<{
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const couponPersentage =
        row.original?.applyCoupon?.discountPercentage || 0
      ...
      const hasDiscount =
        (row.original.originalTotal ?? 0) > (row.original.totalAmount ?? 0)
      return (
    ...
    cell: ({ row }) => {
      return (
        <div className='flex justify-center'>
          <DataTableRowActions
            row={
              row as unknown as import('@tanstack/react-table').Row<OrderRow>
            }
          />
        </div>
      )
