...
const updateProductApi = async (
  ...
  const shouldUseMultipart =
    ...
    ((Array.isArray(jsonPayload.files) && jsonPayload.files.length > 0) ||
      (Array.isArray(jsonPayload.imagesToRemove) &&
        jsonPayload.imagesToRemove.length > 0))
  ...
  if (shouldUseMultipart) {
    ...
    if (jsonPayload.category !== undefined)
      formData.append('category', String(jsonPayload.category))
    if (jsonPayload.name !== undefined)
      formData.append('name', String(jsonPayload.name))
    if (jsonPayload.description !== undefined)
      formData.append('description', String(jsonPayload.description))
    if (jsonPayload.isPremium !== undefined)
      formData.append('isPremium', String(jsonPayload.isPremium))
    if (jsonPayload.isPopular !== undefined)
      formData.append('isPopular', String(jsonPayload.isPopular))
    if (jsonPayload.product_slug !== undefined)
      formData.append('product_slug', String(jsonPayload.product_slug))
    ...
    if (Array.isArray(jsonPayload.ingredients)) {
      jsonPayload.ingredients.forEach((ing) =>
    ...
    const response = await api.put(
      `/products/product/${jsonPayload.id}`,
      formData,
      {
        headers: { 'Content-Type': 'multipart/form-data' },
      }
...
export function useProductsList(params: GetProductsParams) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      getProductsApi({ page, limit, search, isPremium, isPopular }),
